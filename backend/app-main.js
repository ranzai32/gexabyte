...
async function monitorPositionAndPool(tokenId, poolTokenA, poolTokenB, poolFeeTier, walletOrProvider) {
    ...
    try {
        ...
        // 2. Получаем текущее состояние пула
        const currentPool = await getPoolData(poolTokenA, poolTokenB, poolFeeTier); // Используем токены, соответствующие пулу
        if (!currentPool) {
            console.log("Не удалось получить данные пула. Следующая проверка через интервал.");
            return;
        }
        ...
        if (currentPoolTick < Number(positionTickLower)) {
            ...
            // Здесь в будущем будет логика изъятия ликвидности
